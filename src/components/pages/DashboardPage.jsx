  ...
  useEffect(() => {
    const fetchStatsAndDecks = async () => {
      try {
        setLoading(true);
        // Get total decks
        const { count: decksCount, error: decksError, data: decksData } = await supabase
          .from('decks')
          .select('id, title_key', { count: 'exact', head: false });
        if (decksError) throw decksError;
        // Get total cards
        const { count: cardsCount, error: cardsError, data: cardsData } = await supabase
          .from('cards')
          .select('id, deck_id, status', { count: 'exact', head: false });
        if (cardsError) throw cardsError;
        // Get cards ready for image review
        const { count: reviewCount, error: reviewError, data: reviewData } = await supabase
          .from('cards')
          .select('id, deck_id, status, decks!deck_id(id, title_key)', { count: 'exact', head: false })
        ...
        // Decks for review sidebar (reuse logic)
        const titleKeys = [...new Set(reviewData.map(card => card.decks?.title_key))].filter(Boolean);
        ...
        const decksWithTitles = [...new Set(reviewData.map(card => card.deck_id))].map(deckId => {
          const card = reviewData.find(c => c.deck_id === deckId);
        ...
        // Deck stats for table
        const deckStatsArr = decksWithTitles.map(deck => {
          const total = cardsData.filter(card => card.deck_id === deck.id).length;
          const ready = cardsData.filter(card => card.deck_id === deck.id && card.status === 'choosebaseimage').length;
          return {
            ...deck,
            totalCards: total,
            cardsForReview: ready
          };
        });
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-800">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex flex-1 flex-row gap-8 p-8 bg-transparent min-h-0">
          {/* Left: Circular Graphs */}
          <section className="w-72 flex flex-col gap-8 items-center justify-end bg-gradient-to-b from-gray-900 via-gray-950 to-gray-900 rounded-lg p-6 shadow-xl border border-gray-800 min-h-[calc(100vh-180px)] max-h-[calc(100vh-180px)] sticky bottom-0">
            <CircularProgress
              value={decks.length}
          ...
          {/* Center: Decks Table */}
          <section className="flex-1 flex flex-col min-h-[calc(100vh-180px)] max-h-[calc(100vh-180px)] justify-between">
            <div className="flex flex-col h-full">
              ...
              {loading ? (
                ...
                <div className="flex flex-col h-full">
                  <div className="overflow-x-auto rounded-lg shadow border border-gray-800 bg-gradient-to-br from-gray-900 via-gray-950 to-gray-900 flex-1 min-w-0 w-full mx-auto">
                    <table className="min-w-full w-full divide-y divide-gray-800 h-full table-fixed">
                      <thead>
                        <tr>
                          ...
                          <th className="px-2 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider w-1/6">Cards Ready</th>
                          <th className="px-2 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider w-1/6">Total Cards</th>
                          <th className="px-2 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider w-1/6">% Ready</th>
                      ...
                      <tbody className="divide-y divide-gray-800">
                        {paginatedDeckStats.length === 0 && (
                          <tr>
                            <td colSpan={5} className="text-gray-400 text-center py-6">No decks available for review</td>
                          </tr>
                        )}
                        {paginatedDeckStats.map(deck => {
                          const percent = deck.totalCards === 0 ? 0 : Math.round((deck.cardsForReview / deck.totalCards) * 100);
                          return (
                            <tr key={deck.id} className={selectedDeck === deck.id ? 'bg-gray-800/60' : ''}>
                              ...
                              <td className="px-2 py-3 text-gray-100">{deck.cardsForReview}</td>
                              <td className="px-2 py-3 text-gray-100">{deck.totalCards}</td>
                              <td className="px-2 py-3">
                                <span className="inline-block px-2 py-1 rounded-full text-xs font-bold" style={{ background: '#23272f', color: percent > 0 ? '#34d399' : '#f87171' }}>{percent}%</span>
                              </td>
                              <td className="px-2 py-3">
                                <button
